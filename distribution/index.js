...
const Mapper = curry((config, input) => {
  ...
  return Object.keys(input).reduce((final, key) => {
    ...
    if (~keyIndex) {
      ...
      const _config$mappedKey = config[mappedKey],
            { resourceName } = _config$mappedKey,
            props = _objectWithoutProperties(_config$mappedKey, ['resourceName']);
      // Set the returned object's value to be
      // an object with the original value and
      // and of the extra values we wanted to pass
      final[mappedKey] = _extends({
        value: input[key]
      }, props);
    } else {
